    ...
    if clf == "NN":
        ...
        model.steps.append(
            (
                ...
                MLPRegressor(
                    solver="sgd",
                    hidden_layer_sizes=grid_search.best_params_[
        ...
        model = Pipeline([("scalar", StandardScaler()), (clf, reg_dict[clf]())])
    return model
...
def get_best_regressor(
    ...
    for model_name in reg_dict:
        model = Pipeline(
            [("scalar", StandardScaler()), (model_name, reg_dict[model_name]())]
        )
...
def model_choice(clf, xtrain=None, ytrain=None):
    ...
    if clf == "XBG":
        ...
        model = make_pipeline(StandardScaler(), clf_dict[clf](probability=True))
    elif clf == "NN":
        temp_model = Pipeline(
            [
                ("scalar", StandardScaler()),
        ...
        model = Pipeline(
            [
                ...
                (
                    "knn",
                    KNeighborsClassifier(grid_search.best_params_["knn__n_neighbors"]),
                ),
            ]
        )
    else:
